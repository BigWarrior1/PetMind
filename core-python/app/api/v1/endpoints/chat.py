...
回答要求：
- 基于事实，不要编造信息
- 引用知识库来源时，在相关语句后标注 [1]、[2] 等编号（对应上述参考来源编号）
- 语言通俗易懂，适合普通宠物主人理解
"""

# 流式问答系统提示词（知识库无相关内容时使用）
STREAM_FALLBACK_SYSTEM_PROMPT = """你是 PetMind，一个专业的宠物健康顾问。

请基于你的专业知识回答用户关于宠物健康的问题。
    ...
    async def generate():
        ...
        if documents:
            ...
            for i, doc in enumerate(documents, 1):
                ...
                context_parts.append(f"[参考来源 {i}: {source_name}]\n{doc.page_content}")
            context = "\n\n".join(context_parts)
...
请根据上述知识库内容回答。引用时在相关语句后标注参考来源编号，如[1]、[2]。"""
...
请回答用户的问题。"""
            system_prompt = STREAM_FALLBACK_SYSTEM_PROMPT
